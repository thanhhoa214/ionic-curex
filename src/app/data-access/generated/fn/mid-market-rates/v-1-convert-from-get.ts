...
export interface V1ConvertFromGet$Params {
...
/**
 * Comma separated list of to currencies ISO 4217 codes.  This will limit the data returned to only those currencies you are interested in. Use an asterisk * to convert all currencies.</br> Note: Obsolete currencies are replaced by their successor currency.</br>
 */
  to: string;
...
/**
 * OPTIONAL – If ‘true’ then endpoint will include inverse rates. An inverse rate is a quote for which the base currency and counter currency are switched. An inverse is calculated by dividing one by the exchange rate. Example: If the exchange rate for $1 USD to EUR = 0.874852, then the inverse rate would be 1/0.874852 = 1.14305, meaning that US$1.14305 would buy 1 euro.
 */
  inverse?: boolean;
...
export function v1ConvertFromGet(http: HttpClient, rootUrl: string, params: V1ConvertFromGet$Params, context?: HttpContext): Observable<StrictHttpResponse<ConvertFromResponse>> {
  const rb = new RequestBuilder(rootUrl, v1ConvertFromGet.PATH, 'get');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<ConvertFromResponse>;
    })
  );
}

v1ConvertFromGet.PATH = '/v1/convert_from';
